...
# C) Function stru.plot.sd.bci.k2: plots STRUCTURE results with the different types of "confidence" intervals defined above around mean Q values
    # implemented for k=2 only in this version, easily changeable for ggplot2 options

# How to use the script with the examples provided? see the readme.txt online
...
# 3a) Condition to check that nb of replicates is the same in _q and _f files
 if (rep.nb.f!=rep.nb.q) { cat("ERR: number of replicates from _q files and _f files differ", "\n")
 } else {
...
remove.empty.str.in.vec<-function(x){   # removes empty string "" in charvec x to be applied in an vector set in a list for ex
# useful when scanning a file with lines which have been converted into a char vec where numbers exists + blank spaces
# also from a list of char vec to a df with ldply
          el.2del<-NULL
          for (i in 1:length(x)) {
             if (x[i]=="") el.2del<-c(el.2del,i) else el.2del<-el.2del
          }
          y<-x[-el.2del]
          return(y)
}
...
# 3c) Extract Bayesian CI from df list, convert values into numeric values & match CI of same inferred clusters among replicates…
 ...
 # 6) save as text file
 outfile<-paste("mmm.ci-k",k,"-",rep.nb.q,"-",res.id,".txt",sep="")
...
leg.pos=c(c(.85,.15),"none"),title.plot=T,hjust.val=1,vjust.val=1,hplot.out=5,wplot.out=10,silent=F) {

        # Arguments:
        # strudat: output dataframe (df) from stru.out.2.ci or stru.out.2.sd functions
        # plot.type: if "sd"/"bci", it will assume that strudat is a stru.out.2.sd/ci df and either sd/bci around mean Q values are plotted
        # plots use names from given individuals in these df, or change them in these df before
        ...
        # bci.size: % of credible interval considered in the output df from Fun (B)
        ...
        # hplot.out=4: height of pdf plot output ex 4 cm  , impact relative size of text on the plot
        ...
        # but other types such as genetic resampling among available markers as in the Lang et al. 2018 study can be used,
...
if (k==2) {
  ...
  if (plot.type=="sd") {
   if (silent==F) {
     Sys.sleep(0.1); cat(" Plot of individuals mean Q_values with standard deviations across replicates,","\n","assumes input file is from stru.out.2.sd FUN","\n","and a normal distribution of Q-values with ~95% confidence intervals ","\n")
   flush.console() }
    
     # 1a) use "melt" to reformat sd data
    mean.sd.sorted<-strudat[order(strudat$mQval.c2),]
    mqsd.sort<-mean.sd.sorted[,1:(k+1)] # extract mQ values col for melting
    msd.sort<-mean.sd.sorted[,c(1,(k+2))]  # extract sd values
    colnames(msd.sort)[2]<-c("sd.Qval.c1")
    melt.q.sd<-melt(mqsd.sort,id=c("ind"))
    colnames(melt.q.sd)<-c("ind","cluster","mean.Qvalue")

    # 1b) prep ggplot2 files & additions
    meltqsd<-merge(melt.q.sd,msd.sort, by.x="ind",by.y="ind",all.x=T, all.y=T, sort=T)
    meltq.sd.sort<-meltqsd[order(meltqsd$cluster,meltqsd$mean.Qvalue),]
    meltq.sd.sort$ind<-factor(meltq.sd.sort$ind,levels=as.vector(meltq.sd.sort$ind)[1:ind.nb]) # fix the sorting for ggplot2
    meltq.sd.sort[which(meltq.sd.sort$cluster=="mQval.c2"),4]<-NA
   ...
   # 1d) below Axes setup and if legend wanted
    if (ind.nb<=30) {
    my.xaxis<- theme(axis.ticks.x=element_blank(),axis.text.x = element_text(colour="black",angle=as.numeric(ang.xlabel),hjust=hjust.val,vjust=vjust.val,size = rel(0.9)),legend.position = leg.pos)
  ...
  if (plot.type=="bci") {
    ...
    mean.ci.sorted<-strudat[order(strudat$mQval.c1),] # in case k=2 no issue of sorting, here based on first cluster
    ...
    meltq.sort<-meltq[order(meltq$cluster,meltq$mean.Qvalue),]  # sorting by cluster 1 & Q ascending values
    colnames(meltq.sort)[c(4,6,8,10)]<-c("mean.lb.c1", "mean.ub.c1", "min.lb.c1", "max.ub.c1")
    meltq.sort$ind<-factor(meltq.sort$ind,levels=as.vector(meltq.sort$ind)[1:ind.nb])  # fix the sorting for ind in ggplot2
    meltq.sort[which(meltq.sort$cluster=="mQval.c2"),c(4,6,8,10)]<-NA
    meltq.sort<-transform(meltq.sort, mean.Qvalue=1-mean.Qvalue)  # for matching mean.Qvalue and error bar plot
   # 2c) Plot with "ggplot2"
    # plot with m.ci as geom_bar
    if (mean.ci.bar==T) {
...
geom_bar(stat="identity")+ geom_errorbar(aes(ymin =mean.lb.c1, ymax =mean.ub.c1 ), width=0.4, size=0.75)
    } else { # with min/max.ci as geom_bar
p<-ggplot(data=meltq.sort, aes(x=ind, y=mean.Qvalue, fill=cluster)) +
geom_bar(stat="identity") + geom_errorbar(data=meltq.sort, aes(ymin = min.lb.c1, ymax = max.ub.c1), width=0.4, size=0.75) #+
    }
   # Background and title setup
...
if (ind.nb<=30) {
my.xaxis<- theme(axis.ticks.x=element_blank(),axis.text.x = element_text(colour="black",angle=as.numeric(ang.xlabel),
hjust=hjust.val,vjust=vjust.val,size = rel(0.9)),legend.position = leg.pos)
} else if (ind.nb>30 & ind.nb<=60) {
my.xaxis<- theme(axis.ticks.x=element_blank(),axis.text.x = element_text(colour="black",angle=as.numeric(ang.xlabel),
hjust=hjust.val,vjust=vjust.val,size = rel(0.7)),legend.position = leg.pos)
} else if (ind.nb>60 & ind.nb<=150) {
my.xaxis<- theme(axis.ticks.x=element_blank(),axis.text.x = element_text(colour="black",angle=as.numeric(ang.xlabel),
hjust=hjust.val,vjust=vjust.val,size = rel(0.5)),legend.position = leg.pos)
}
...
return(stru.plot)
         ...
         Sys.sleep(0.1); cat("Current implementation of plot script only available for k=2","\n")
         flush.console() }
       }
} # end FUN

#### Example of use for Function stru.out.2.sd (A) : just comment out the line below
...
# and corresponding application of Function stru.plot.sd.bci (C) ie arg value correspond to mean.sd above (just comment out the 3 lines below)
# ex.plot<-stru.plot.sd.bci(strudat=mean.sd,plot.type="sd",mean.ci.bar=T,ind.nb=24,rep.nb=10,k=2,pol.nb=541,pol.type="SNPs",
# samp.set="random",ang.xlabel=75,col.title="black",col.cluster=c("blue","green"),leg.pos=c(.8,.4),title.plot=T,hjust.val=0.65,vjust.val=NULL,
# hplot.out=4,wplot.out=6,silent=F)

#### Example of use for Function stru.out.2.ci (B) :
...
# and corresponding application of Function stru.plot.sd.bci (C) ie arg value correspond to outci above
ex.plot<-stru.plot.sd.bci(strudat=outci,plot.type="bci",mean.ci.bar=T,ind.nb=24,rep.nb=10,k=2,pol.nb=541,
pol.type="SNPs",samp.set="random",ang.xlabel=75,col.title="black",col.cluster=c("blue","pink"),leg.pos=c(.8,.4),
title.plot=T,hjust.val=0.9,vjust.val=01,hplot.out=4,wplot.out=6,silent=F)
